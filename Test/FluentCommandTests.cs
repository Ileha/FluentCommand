...
namespace Test
{
    ...
    public class FluentCommandTests
    {
        ...
        public void ComplicatedCommand_ShouldCreateCommand()
        {
            ...
            // Assert
            Assert.That(result, Is.EqualTo("git clone --branch release/v1.2 --depth 10 --single-branch --recurse-submodules --shallow-submodules --config core.eol=lf --config fetch.prune=true https://github.com/user/repo.git ./repo"));
        }
        ...
        public void Test_NestedCommandEscaping()
        {
            ...
            // Assert
            Assert.That(result, Is.EqualTo("bash -c \"udocker run container bash -c \\\"echo \\\\\\\"hello world\\\\\\\"\\\"\""));
        }
        ...
        public void Test_BuildFluentCommand()
        {
            ...
            Assert.That(result, Is.EqualTo("run -e data=Development -e product=Production --user=user container bash -c \"apt-get update && apt-get install -y curl sudo\""));
        }
    }
}